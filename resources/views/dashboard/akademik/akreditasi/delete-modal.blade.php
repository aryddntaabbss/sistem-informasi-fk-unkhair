    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="staticBackdropLabel">Hapus Data Akreditasi</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form method="post" action="{{ route('delete-akreditasi', $a->id) }}">
                @csrf
                @method('delete')
                <div class="modal-body">
                    <p>Apakah anda yakin akan menghapus data akreditasi "{{ $a->nama_prodi }}" ?</p>
                </div>
